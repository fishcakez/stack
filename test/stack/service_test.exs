defmodule Stack.ServiceTest do
  ...
  test "map rescues exception" do
    ...
    assert {:error, %RuntimeError{}} = Service.call(service, 1)
  end
  ...
  test "merge runs first service and then second" do
    ...
    service = Service.merge(service1, service2)

    assert Service.call(service, 1) == 4
  end

  test "merge runs first service, maps value and then second" do
    service1 =
      Service.new()
      |> Service.map(fn n -> n + 1 end)
    ...
    service = Service.merge(service1, service2, fn n -> div(n, 2) end)

    assert Service.call(service, 5) == 9
  end
