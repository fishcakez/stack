defmodule Stack.Criticality do
  ...
  """
  @type t :: :critical_plus | :critical | :scheddable_plus | :sheddable
  ...
  def compare_levels(:sheddable_plus, :scheddable), do: :gt
  def compare_levels(:sheddable_plus, :scheddable_plus), do: :eq
  def compare_levels(:sheddable_plus, _), do: :lt
  def compare_levels(:sheddable, :scheddable_plus), do: :lt
  def compare_levels(:sheddable, :scheddable), do: :eq
  
  @doc """
  Bind a criticality level to the scope of an anonymouns function and run the function.

  The first argument is a criticality level. If a criticality level is current bound the
  greatest level takes priority.
  ...
  def bind(criticality, fun) when criticality in [:critical_plus, :critical, :scheddable_plus, :sheddable] do
    current = Context.get(Criticality, :scheddable)
    case compare_levels(current, criticality) do
      :lt ->
        fun.()
      _ ->
        Context.bind(criticality, fun)
    end
  end
